...
subprocess â Subprocess management — Python 3.14.2 documentation
Navigation
index
modules |
next |
...
Dark
|
subprocess â Subprocess managementÂ¶
...
this function are passed through to that interface. (timeout,
input,
...
subprocessâs stdin.
If used it must be a byte sequence, or a string if
encoding or errors is specified or text is true.
When
...
The universal_newlines argument is equivalent
to text and is provided
...
longer works.
class subprocess.CompletedProcessÂ¶
...
output.
exception subprocess.SubprocessErrorÂ¶
Base class for all other exceptions from this module.
Added in version 3.3.
exception subprocess.TimeoutExpiredÂ¶
...
the shell.
This can be useful if you are using Python primarily for the
enhanced control flow it offers over most system shells and still want
convenient access to other shell features such as shell pipes, filename
wildcards, environment variable expansion, and expansion of ~ to a
userâs home directory.
However, note that Python itself offers
...
os.execvpe()-like behavior to execute the child program.
On Windows,
...
a sequence.
If args is a string, the interpretation is
platform-dependent and described below.
See the shell and executable
arguments for additional differences from the default behavior.
Unless
...
manner described in Converting an argument sequence to a string on Windows.
This is because
...
the shell as the program to execute.
If shell is True, it is
...
formatted exactly as it would be when typed at the shell prompt.
This
includes, for example, quoting or backslash escaping filenames with spaces in
them.
If args is a sequence, the first item specifies the command string, and
...
into the shell (e.g. dir or copy).
You do not need
...
When shell=False, executable replaces the
program to execute specified by args.
However, the original args is
still passed to the program.
Most programs treat the program specified
by args as the command name, which can then be different from the program
actually executed.
On POSIX, the args name
becomes the display name for the executable in utilities such as
ps.
If shell=True, on POSIX the executable argument
...
2 will be closed before the child process is executed.
Otherwise
...
True when redirecting the standard handles. Itâs now possible to
set close_fds to True when redirecting the standard handles.
pass_fds is an optional sequence of file descriptors to keep open
between the parent and child.
Providing any pass_fds forces
...
execute.
On Windows, in order to run a side-by-side assembly the
...
implicitly choose to call a system shell.
...
It is safe to catch this exception and
retry the wait.
Note
...
Popen.communicate(input=None, timeout=None)Â¶
Interact with process: Send data to stdin.
Read data from stdout and stderr,
until end-of-file is reached.
Wait for process to terminate and set the
returncode attribute.
The optional input argument should be
...
Popen.argsÂ¶
The args argument as it was passed to Popen â a
...
A negative value -N indicates that the child was terminated by signal
N (POSIX only).
Windows Popen HelpersÂ¶
The STARTUPINFO class and following constants are only available
on Windows.
class subprocess.STARTUPINFO(*, dwFlags=0, hStdInput=None, hStdOutput=None, hStdError=None, wShowWindow=0, lpAttributeList=None)Â¶
...
subprocess.ABOVE_NORMAL_PRIORITY_CLASSÂ¶
A Popen creationflags parameter to specify that a new process
will have an above average priority.
Added in version 3.7.
subprocess.BELOW_NORMAL_PRIORITY_CLASSÂ¶
A Popen creationflags parameter to specify that a new process
will have a below average priority.
Added in version 3.7.
subprocess.HIGH_PRIORITY_CLASSÂ¶
A Popen creationflags parameter to specify that a new process
will have a high priority.
Added in version 3.7.
subprocess.IDLE_PRIORITY_CLASSÂ¶
A Popen creationflags parameter to specify that a new process
will have an idle (lowest) priority.
Added in version 3.7.
subprocess.NORMAL_PRIORITY_CLASSÂ¶
A Popen creationflags parameter to specify that a new process
will have a normal priority. (default)
Added in version 3.7.
subprocess.REALTIME_PRIORITY_CLASSÂ¶
A Popen creationflags parameter to specify that a new process
...
Added in version 3.7.
subprocess.CREATE_NO_WINDOWÂ¶
A Popen creationflags parameter to specify that a new process
will not create a window.
Added in version 3.7.
subprocess.DETACHED_PROCESSÂ¶
A Popen creationflags parameter to specify that a new process
will not inherit its parentâs console.
This value cannot be used with CREATE_NEW_CONSOLE.
Added in version 3.7.
subprocess.CREATE_DEFAULT_ERROR_MODEÂ¶
A Popen creationflags parameter to specify that a new process
...
Added in version 3.7.
subprocess.CREATE_BREAKAWAY_FROM_JOBÂ¶
A Popen creationflags parameter to specify that a new process
is not associated with the job.
Added in version 3.7.
Older high-level APIÂ¶
...
function.
The child process will block if it generates enough
...
malicious program named cmd.exe into a current directory no
longer works.
subprocess.check_call(args, *, stdin=None, stdout=None, stderr=None, shell=False, cwd=None, timeout=None, **other_popen_kwargs)Â¶
Run command with arguments.
Wait for command to complete. If the return
...
malicious program named cmd.exe into a current directory no
longer works.
subprocess.check_output(args, *, stdin=None, stderr=None, shell=False, cwd=None, encoding=None, errors=None, universal_newlines=None, timeout=None, text=None, **other_popen_kwargs)Â¶
...
malicious program named cmd.exe into a current directory no
longer works.
Replacing Older Functions with the subprocess ModuleÂ¶
...
executed program cannot be found; the âbâ replacements raise OSError
instead.
...
Replacing /bin/sh shell command substitutionÂ¶
output=$(mycmd myarg)
becomes:
output = check_output(["mycmd", "myarg"])
Replacing shell pipelineÂ¶
...
Replacing the os.spawn familyÂ¶
P_NOWAIT example:
...
if process.wait() != 0:
print("There were some errors")
Legacy Shell Invocation FunctionsÂ¶
This module also provides the following legacy functions from the 2.x
commands module. These operations implicitly invoke the system shell and
none of the guarantees described above regarding security and exception
handling consistency are valid for these functions.
subprocess.getstatusoutput(cmd, *, encoding=None, errors=None)Â¶
...
subprocess.getoutput(cmd, *, encoding=None, errors=None)Â¶
Return output (stdout and stderr) of executing cmd in a shell.
Like getstatusoutput(), except the exit code is ignored and the return
value is a string containing the commandâs output.
Example:
...
Converting an argument sequence to a string on WindowsÂ¶
