...
sqlite3 â DB-API 2.0 interface for SQLite databases — Python 3.14.2 documentation
Navigation
index
modules |
next |
...
Dark
|
sqlite3 â DB-API 2.0 interface for SQLite databasesÂ¶
...
Itâs also possible to prototype an
application using SQLite and then port the code to a larger database such as
PostgreSQL or Oracle.
The sqlite3 module was written by Gerhard HÃ¤ring.
...
PEP 249 - Database API Specification 2.0PEP written by Marc-AndrÃ© Lemburg.
TutorialÂ¶
...
as the query returns a tuple containing the tableâs name.
...
ReferenceÂ¶
Module functionsÂ¶
sqlite3.connect(database, timeout=5.0, detect_types=0, isolation_level='DEFERRED', check_same_thread=True, factory=sqlite3.Connection, cached_statements=128, uri=False, *, autocommit=sqlite3.LEGACY_TRANSACTION_CONTROL)Â¶
Open a connection to an SQLite database.
Parameters:
database (path-like object) â The path to the database file to be opened.
You can pass ":memory:" to create an SQLite database existing only
in memory, and open a connection
to it.
timeout (float) â How many seconds the connection should wait before raising
...
detect_types (int) â Control whether and how data types not
...
isolation_level (str | None) â Control legacy transaction handling behaviour.
...
factory (Connection) â A custom subclass of Connection to create the connection with,
if not the default Connection class.
cached_statements (int) â The number of statements that sqlite3
should internally cache for this connection, to avoid parsing overhead.
By default, 128 statements.
uri (bool) â If set to True, database is interpreted as a
...
autocommit (bool) â Control PEP 249 transaction handling behaviour.
...
sqlite3.complete_statement(statement)Â¶
...
for real-world use.
sqlite3.enable_callback_tracebacks(flag, /)Â¶
...
sqlite3.register_adapter(type, adapter, /)Â¶
...
sqlite3.register_converter(typename, converter, /)Â¶
...
case-insensitively.
Module constantsÂ¶
sqlite3.LEGACY_TRANSACTION_CONTROLÂ¶
...
! will look up a converter named "integer"
p point,
! will look up a converter named "point"
n number(10)
! will look up a converter named "number"
)
...
SELECT MAX(p) as "p [point]" FROM test;
! will look up converter "point"
This flag may be combined with PARSE_DECLTYPES using the |
(bitwise or) operator.
sqlite3.SQLITE_OKÂ¶
sqlite3.SQLITE_DENYÂ¶
sqlite3.SQLITE_IGNOREÂ¶
...
sqlite3.SQLITE_DBCONFIG_DEFENSIVEÂ¶
sqlite3.SQLITE_DBCONFIG_DQS_DDLÂ¶
sqlite3.SQLITE_DBCONFIG_DQS_DMLÂ¶
sqlite3.SQLITE_DBCONFIG_ENABLE_FKEYÂ¶
sqlite3.SQLITE_DBCONFIG_ENABLE_FTS3_TOKENIZERÂ¶
sqlite3.SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSIONÂ¶
sqlite3.SQLITE_DBCONFIG_ENABLE_QPSGÂ¶
sqlite3.SQLITE_DBCONFIG_ENABLE_TRIGGERÂ¶
sqlite3.SQLITE_DBCONFIG_ENABLE_VIEWÂ¶
sqlite3.SQLITE_DBCONFIG_LEGACY_ALTER_TABLEÂ¶
sqlite3.SQLITE_DBCONFIG_LEGACY_FILE_FORMATÂ¶
sqlite3.SQLITE_DBCONFIG_NO_CKPT_ON_CLOSEÂ¶
sqlite3.SQLITE_DBCONFIG_RESET_DATABASEÂ¶
sqlite3.SQLITE_DBCONFIG_TRIGGER_EQPÂ¶
sqlite3.SQLITE_DBCONFIG_TRUSTED_SCHEMAÂ¶
sqlite3.SQLITE_DBCONFIG_WRITABLE_SCHEMAÂ¶
...
blobopen(table, column, rowid, /, *, readonly=False, name='main')Â¶
Open a Blob handle to an existing
BLOB.
Parameters:
table (str) â The name of the table where the blob is located.
column (str) â The name of the column where the blob is located.
rowid (int) â The row id where the blob is located.
readonly (bool) â Set to True if the blob should be opened without write
permissions.
Defaults to False.
name (str) â The name of the database where the blob is located.
Defaults to "main".
Raises:
OperationalError â When trying to open a blob in a WITHOUT ROWID table.
Return type:
Blob
Note
...
execute(sql, parameters=(), /)Â¶
Create a new Cursor object and call
execute() on it with the given sql and parameters.
Return the new cursor object.
executemany(sql, parameters, /)Â¶
Create a new Cursor object and call
executemany() on it with the given sql and parameters.
Return the new cursor object.
executescript(sql_script, /)Â¶
Create a new Cursor object and call
executescript() on it with the given sql_script.
Return the new cursor object.
create_function(name, narg, func, *, deterministic=False)Â¶
Create or remove a user-defined SQL function.
Parameters:
name (str) â The name of the SQL function.
narg (int) â The number of arguments the SQL function can accept.
If -1, it may take any number of arguments.
func (callback | None) â A callable that is called when the SQL function is invoked.
The callable must return a type natively supported by SQLite.
Set to None to remove an existing SQL function.
deterministic (bool) â If True, the created SQL function is marked as
deterministic,
...
create_aggregate(name, n_arg, aggregate_class)Â¶
Create or remove a user-defined SQL aggregate function.
Parameters:
name (str) â The name of the SQL aggregate function.
n_arg (int) â The number of arguments the SQL aggregate function can accept.
If -1, it may take any number of arguments.
aggregate_class (class | None) â A class must implement the following methods:
...
These parameters will become positional-only in Python 3.15.
create_window_function(name, num_params, aggregate_class, /)Â¶
Create or remove a user-defined aggregate window function.
Parameters:
name (str) â The name of the SQL aggregate window function to create or remove.
num_params (int) â The number of arguments the SQL aggregate window function can accept.
If -1, it may take any number of arguments.
aggregate_class (class | None) â A class that must implement the following methods:
...
Raises:
NotSupportedError â If used with a version of SQLite older than 3.25.0,
...
print(cur.fetchall())
con.close()
create_collation(name, callable, /)Â¶
...
Changed in version 3.11: The collation name can contain any Unicode character.
Earlier, only
...
set_authorizer(authorizer_callback)Â¶
...
set_progress_handler(progress_handler, n)Â¶
...
The parameter will become positional-only in Python 3.15.
set_trace_callback(trace_callback)Â¶
...
The parameter will become positional-only in Python 3.15.
enable_load_extension(enabled, /)Â¶
...
aggregates or whole new virtual table implementations.
One well-known
...
print(row)
load_extension(path, /, *, entrypoint=None)Â¶
Load an SQLite extension from a shared library.
Enable extension loading with enable_load_extension() before
calling this method.
Parameters:
path (str) â The path to the SQLite extension.
entrypoint (str | None) â Entry point name.
If None (the default),
...
backup(target, *, pages=-1, progress=None, name='main', sleep=0.250)Â¶
Create a backup of an SQLite database.
Works even if the database is being accessed by other clients
or concurrently by the same connection.
Parameters:
target (Connection) â The database connection to save the backup to.
pages (int) â The number of pages to copy at a time.
If equal to or less than 0,
the entire database is copied in a single step.
Defaults to -1.
progress (callback | None) â If set to a callable,
...
Defaults to None.
name (str) â The name of the database to back up.
...
sleep (float) â The number of seconds to sleep between successive attempts
...
Parameters:
category (int) â The SQLite limit category to be queried.
Return type:
int
Raises:
ProgrammingError â If category is not recognised by the underlying SQLite library.
Example, query the maximum length of an SQL statement
for Connection con (the default is 1000000000):
>>> con.getlimit(sqlite3.SQLITE_LIMIT_SQL_LENGTH)
1000000000
Added in version 3.11.
setlimit(category, limit, /)Â¶
...
Parameters:
category (int) â The SQLite limit category to be set.
limit (int) â The value of the new limit.
...
Parameters:
op (int) â A SQLITE_DBCONFIG code.
Return type:
bool
Added in version 3.12.
setconfig(op, enable=True, /)Â¶
Set a boolean connection configuration option.
Parameters:
op (int) â A SQLITE_DBCONFIG code.
enable (bool) â True if the configuration option should be enabled (default);
False if it should be disabled.
Added in version 3.12.
serialize(*, name='main')Â¶
Serialize a database into a bytes object.
For an
ordinary on-disk database file, the serialization is just a copy of the
disk file.
For an in-memory database or a âtempâ database, the
serialization is the same sequence of bytes which would be written to
disk if that database were backed up to disk.
Parameters:
name (str) â The database name to be serialized.
Defaults to "main".
Return type:
bytes
Note
...
Parameters:
data (bytes) â A serialized database.
name (str) â The database name to deserialize into.
Defaults to "main".
Raises:
OperationalError â If the database connection is currently involved in a read
transaction or a backup operation.
DatabaseError â If data does not contain a valid SQLite database.
OverflowError â If len(data) is larger than 2**63 - 1.
Note
This method is only available if the underlying SQLite library has the
deserialize API.
Added in version 3.11.
...
True: Use SQLiteâs autocommit mode.
...
placeholders.
Parameters:
sql (str) â A single SQL statement.
parameters (dict | sequence) â Python values to bind to placeholders in sql.
...
Raises:
ProgrammingError â When sql contains more than one SQL statement.
...
Parameters:
sql (str) â A single SQL DML statement.
parameters (iterable) â An iterable of parameters to bind with
...
fetchmany(size=cursor.arraysize)Â¶
...
setinputsizes(sizes, /)Â¶
Required by the DB-API. Does nothing in sqlite3.
setoutputsize(size, column=None, /)Â¶
Required by the DB-API. Does nothing in sqlite3.
arraysizeÂ¶
...
change the blob length.
Writing beyond the end of the blob will raise
ValueError.
tell()Â¶
Return the current access position of the blob.
seek(offset, origin=os.SEEK_SET, /)Â¶
...
current position) and os.SEEK_END (seek relative to the blobâs
end).
PrepareProtocol objectsÂ¶
class sqlite3.PrepareProtocolÂ¶
The PrepareProtocol typeâs single purpose is to act as a PEP 246 style
adaption protocol for objects that can adapt themselves to native SQLite types.
ExceptionsÂ¶
The exception hierarchy is defined by the DB-API 2.0 (PEP 249).
exception sqlite3.WarningÂ¶
...
from the SQLite API
Added in version 3.11.
exception sqlite3.InterfaceErrorÂ¶
...
exception sqlite3.DatabaseErrorÂ¶
...
exception sqlite3.OperationalErrorÂ¶
...
exception sqlite3.IntegrityErrorÂ¶
Exception raised when the relational integrity of the database is affected,
e.g. a foreign key check fails.
It is a subclass of DatabaseError.
exception sqlite3.InternalErrorÂ¶
...
exception sqlite3.ProgrammingErrorÂ¶
...
exception sqlite3.NotSupportedErrorÂ¶
...
Default adapters and converters (deprecated)Â¶
Note
...
ISO 8601 format.
A converter for declared âdateâ types to
...
using the interpreterâs -m switch,
...
Added in version 3.12.
How-to guidesÂ¶
How to use placeholders to bind values in SQL queriesÂ¶
...
argument of the cursorâs execute() method.
...
How to adapt custom Python types to SQLite valuesÂ¶
...
How to write adaptable objectsÂ¶
...
print(cur.fetchone()[0])
con.close()
How to register adapter callablesÂ¶
...
print(cur.fetchone()[0])
con.close()
How to convert SQLite values to custom Python typesÂ¶
...
cur.close()
con.close()
Adapter and converter recipesÂ¶
...
How to use connection shortcut methodsÂ¶
Using the execute(),
...
con.close()
How to use the connection context managerÂ¶
...
How to create and use row factoriesÂ¶
...
How to handle non-UTF-8 text encodingsÂ¶
...
Because of SQLiteâs flexible typing, it is not uncommon to encounter table
...
Transaction control via the autocommit attributeÂ¶
...
Set autocommit to True to enable SQLiteâs autocommit mode.
...
See Transaction control via the isolation_level attribute for more information.
Transaction control via the isolation_level attributeÂ¶
Note
...
You can choose the underlying SQLite transaction behaviour â
...
Please donate.
Last updated on Jan 13, 2026 (00:21 UTC).
Found a bug?
